...
enum speakerStates{SpeakerOn, SpeakerOff, SpeakerWait}speakerState;
void SpeakerSM_Tick(){
	tmpA = ~PINA&0x04; //PA2 decides on or off position
	switch(speakerState){
		case SpeakerOn:
			if(tmpA){
				speakerState = SpeakerOff; //toggles on and off for 3ms
			}
			else{
				speakerState = SpeakerWait;
			}
			break;
		case SpeakerOff:
			if(tmpA){
...
void CombineLedsSM_Tick(){
	...
	switch(combineState){
		case combination:
			tmpB = BlinkingLeds | ThreeLeds | Speakers;
		break;
	}
}
int main(void) {
	...
	unsigned long TimerThreeLeds = 300;
	const unsigned long SpeakerToggle = 2;
	unsigned long TimerBlink = 1000;
	unsigned long Time = 0; //
	unsigned long SpeakerTime = 0;
	TimerSet(SpeakerToggle);
	TimerOn();
	threeState = start;
	...
	TimerFlag = 0;
	Time += SpeakerToggle; //setting our timers to compare with the toggle which is 2ms
	SpeakerTime += SpeakerToggle;
    }
    return 0;
}
